import { getToken, removeToken, saveToken } from "@/config/localStorage"
...
const actions = {
  ...
  async sendLoginRequest({ commit }, login) {
    ...
    return await api.loginRequest(login).then(({ data }) => {
  ...
  async sendVerifyRequest({ dispatch }, hash) {
    return await api.verifyRequest(hash).then(() => dispatch("getUserData"))
  }
}
